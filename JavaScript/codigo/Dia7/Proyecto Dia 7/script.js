...
function calcular(){
    ...
    //emepzamos loop para comparar inputs
    for (let parr of elementoVentas.children){
        let input = parr.children[1] //usamos parr.children
        input.setAttribute("class","normal")
        if (parseInt(input.value) == ventaMayor){
            input.setAttribute("class","ventaMayor")
        }else if (parseInt(input.value) == ventaMenor){
                input.setAttribute("class","ventaMenor")
        }
    }
}
