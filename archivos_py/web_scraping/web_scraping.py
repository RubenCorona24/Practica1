...
def extraer_titulos(html): #De argumento recibe al html
    ...
    return titulos #Devuelve los títulos

def extraer_articulos(html): #Toma como argumento el html  
    soup = BeautifulSoup(html,'html.parser')
    #lista para almacenar los articulos
    articulos = []
    for articulo_elemen in soup.select('article, .article, .post, .news-item'):
        articulo= {} #Creamos un diccionario


html = obtener_html('https://www.jornada.com.mx/') #Nos devuelve el html
print(extraer_titulos(html)) #Con la función extraer títulos, devuelve  xla lista de títulos 
